...
# User variables
file_name = 'Cotinine Urine 20190812' # Input Excel filename
...
#%% Initial full calibration: Calibration curve (regression)
...
#%% Initial full calibration: IS recovery plot
...
plt.legend(loc = 2)
plt.suptitle(file_name, x = 0.1, y = -0.3, ha = 'left', size = 10)
...
#%% Initial full calibration: TC concentration plot
plt.figure(figsize=(15,3.5))
...
#Selection_cal = df_cal_1['Response_Ratio'].tail(1).item() # use for selection in df_spl
...
df_spl_1 = df_spl.loc[df_spl['Response_Ratio'] <= Selection_cal, ['SampleID', 'TC_Response', 'IS_Response', 'Response_Ratio']]
df_spl_2 = df_spl.loc[df_spl['Response_Ratio'] > Selection_cal, ['SampleID', 'TC_Response', 'IS_Response', 'Response_Ratio']]

#%% Quantificaton of first set: Linear model
...
#%% Quantification of second set: Linear model
...
#%% Plots: Calibration curves for first and second sets
...
df_spl_combined_export = df_spl_combined_export.loc[:, ['Batch', 'SampleID', 'IS_Recovery', 'Measured_TC_Conc', 'Cotinine in Urine (ng cotinine / mL urine']]
